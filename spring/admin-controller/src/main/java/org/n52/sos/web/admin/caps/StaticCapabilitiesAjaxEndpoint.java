...
public class StaticCapabilitiesAjaxEndpoint extends AbstractAdminCapabiltiesAjaxEndpoint {
    ...
    public String getStaticCapabilities() throws JSONException, SQLException, OwsExceptionReport {
        JSONObject response = new JSONObject();
        String current = getSelectedStaticCapabilities();
        JSONObject staticCapabilities = new JSONObject();
        for (StaticCapabilities sc : getDao().getStaticCapabilities().values()) {
            staticCapabilities.put(sc.getIdentifier(), sc.getDocument());
        }
        response.put(STATIC_CAPABILITIES, staticCapabilities);
    ...
    public void setCurrentCapabilities(@RequestBody String json) throws JSONException, SQLException,
                                                                        ConfigurationException, OwsExceptionReport,
                                                                        NoSuchExtensionException {
        String id = null;
        Object currentAttr = new JSONObject(json).opt(CURRENT);
        if (currentAttr != null && currentAttr != JSONObject.NULL) {
            id = String.valueOf(currentAttr).trim();
        }
