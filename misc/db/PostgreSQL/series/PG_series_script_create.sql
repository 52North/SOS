...
create table public.series (seriesId int8 not null, featureOfInterestId int8 not null, observablePropertyId int8 not null, procedureId int8 not null, offeringId int8, deleted char(1) default 'F' not null check (deleted in ('T','F')), published char(1) default 'T' not null check (published in ('T','F')), firstTimeStamp timestamp default NULL, lastTimeStamp timestamp default NULL, firstNumericValue float8, lastNumericValue float8, unitId int8, primary key (seriesId));
