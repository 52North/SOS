...
public class ContactsEnrichment extends SensorMLEnrichment {
    private static final Logger LOGGER = LoggerFactory
            .getLogger(ContactsEnrichment.class);

    @Override
    protected void enrich(AbstractSensorML description) throws
            OwsExceptionReport {
        // set contacts --> take from service information
        if (!description.isSetContact()) {
            Optional<SmlResponsibleParty> contact
                    = createContactFromServiceContact();
    ...
    Optional<SosServiceProvider> getServiceProvider() {
        try {
            return Optional.fromNullable(Configurator.getInstance()
                    .getServiceProvider());
        } catch (final OwsExceptionReport e) {
            LOGGER
                    .error(String.format("Exception thrown: %s", e.getMessage()), e);
