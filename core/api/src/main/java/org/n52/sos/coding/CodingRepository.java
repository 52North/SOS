...
public class CodingRepository {
	...
	private final Map<ProcedureDescriptionFormatKey, Boolean> transactionalProcedureDescriptionFormatsStatus = Maps
			.newHashMap();
	...
	private void generateResponseFormatMaps() {
		...
		final Set<ServiceOperatorKey> serviceOperatorKeyTypes = ServiceOperatorRepository.getInstance()
	...
	private void generateProcedureDescriptionFormatMaps() {
		...
		for (final Encoder<?, ?> e : getEncoders()) {
			if (e instanceof ProcedureEncoder) {
				...
				for (final ServiceOperatorKey sokt : serviceOperatorKeyTypes) {
					final Set<String> rfs = oe.getSupportedProcedureDescriptionFormats(sokt.getService(),
							sokt.getVersion());
					if (rfs != null) {
						for (final String rf : rfs) {
	...
	public Set<String> getSupportedTransactionalProcedureDescriptionFormats(final String service,
		...
		return getSupportedProcedureDescriptionFormats(service, version, transactionalProcedureDescriptionFormatsStatus,
				transactionalProcedureDescriptionFormats);
	}
