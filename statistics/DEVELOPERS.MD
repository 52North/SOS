...
use it like this:
```java
//handlers is a `Map<String,IServiceEventHandler<T>>` and are previously injected vis SpringFramework
//response is the payload of the event (or in some cases the event class itself)
