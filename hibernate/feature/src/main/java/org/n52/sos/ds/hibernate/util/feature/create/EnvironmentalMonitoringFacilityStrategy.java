...
//              emFeature.addObservingCapability(createObservingCapability(child.getIdentifier()));
    ...
    private ObservingCapability createObservingCapability(String identifier) {
        String gdaUrl = "...&featureOfInterest=" + identifier;
    ...
    private void addHasObservations(org.n52.svalbard.inspire.ef.EnvironmentalMonitoringFacility emFeature,
            FeatureOfInterest feature) {
        createAndAddHasObservations(emFeature, feature);
        // check for child features
        if (feature.hasChilds()) {
            for (FeatureOfInterest child : feature.getChilds()) {
                createAndAddHasObservations(emFeature, child);
            }
        }
    }

    private void createAndAddHasObservations(org.n52.svalbard.inspire.ef.EnvironmentalMonitoringFacility emFeature,
            FeatureOfInterest feature) {
        Set<String> offerings = getCache().getOfferingsForFeatureOfInterest(feature.getIdentifier());
        if (offerings != null) {
            for (String offering : offerings) {
//                emFeature.addHasObservation(createObservation(feature, offering));
            }
        }
    }

    private OmObservation createObservation(FeatureOfInterest feature, String offering) {
        String getObservationURL = "...&offering=" + offering;
