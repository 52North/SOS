...
public abstract class AbstractSosResponseEncoder<T extends AbstractServiceResponse> extends AbstractResponseEncoder<T> {
    ...
    protected void createExtension(ExtensibleResponseType xbResponse, SwesExtensions extensions) throws OwsExceptionReport {
        ...
        for (SwesExtension<?> extension : extensions.getExtensions()) {
            if (extension.getValue() instanceof SweAbstractDataComponent) {
                xbResponse.addNewExtension()
                        .set(encodeSwe(helperValues, extension.getValue()));
            }
        }
    }
}
