...
create table dbo.series (seriesId bigint identity not null, featureOfInterestId bigint not null, observablePropertyId bigint not null, procedureId bigint not null, offeringId bigint, deleted char(1) default 'F' not null check (deleted in ('T','F')), published char(1) default 'T' not null check (published in ('T','F')), firstTimeStamp datetime2 default NULL, lastTimeStamp datetime2 default NULL, firstNumericValue double precision, lastNumericValue double precision, unitId bigint, primary key (seriesId));
