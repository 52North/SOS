...
public class GmlEncoderv321 extends AbstractXmlEncoder<Object> {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(GmlEncoderv321.class);
	...
	private static final Set<EncoderKey> ENCODER_KEY_TYPES = CodingHelper
			.encoderKeysForElements(GmlConstants.NS_GML_32,
					...
					org.n52.sos.ogc.gml.CodeType.class, SamplingFeature.class,
					SosEnvelope.class, FeatureCollection.class,
					AbstractGeometry.class);

	public GmlEncoderv321() {
		LOGGER.debug(
				"Encoder for the following keys initialized successfully: {}!",
	...
	public void addNamespacePrefixToMap(
		...
		nameSpacePrefixMap.put(GmlConstants.NS_GML_32,
	...
	@Override
	public XmlObject encode(final Object element,
		...
		if (element instanceof Time) {
			...
			encodedObject = createFeaturePropertyType(
					(AbstractFeature) element, additionalValues);
		} else if (element instanceof GeometryValue) {
			encodedObject = createGeomteryPropertyType(
					(AbstractGeometry) element, additionalValues);
	...
	private XmlObject createFeaturePropertyType(final AbstractFeature feature,
		...
		if (feature instanceof FeatureCollection) {
			return createFeatureCollection((FeatureCollection) feature,
					additionalValues);
	...
	private XmlObject createFeatureCollection(final FeatureCollection element,
		...
		final FeatureCollectionType featureCollection = featureCollectionDoc
...
//			}
		...
		return XmlHelper.substituteElement(
				featurePropertyType.getAbstractFeature(), featurePropertyType);
		// return featureCollection;
	}

	private boolean checkAndPrepareHelperValues(
	...
	private XmlObject createTime(final Time time,
			final Map<HelperValues, String> additionalValues)
			throws OwsExceptionReport {
		if (time != null) {
			if (time instanceof TimeInstant) {
				if (additionalValues.containsKey(HelperValues.DOCUMENT)) {
					return createTimeInstantDocument((TimeInstant) time);
						...
						.containsKey(HelperValues.PROPERTY_TYPE)) {
	...
	private XmlObject createTimePeriodDocument(final TimePeriod time)
			throws OwsExceptionReport {
		final TimePeriodDocument timePeriodDoc = TimePeriodDocument.Factory
				.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
		createTimePeriodType(time, timePeriodDoc.addNewTimePeriod());
		return timePeriodDoc;
	}

	private XmlObject createTimePeriodPropertyType(final TimePeriod time)
			throws OwsExceptionReport {
		final TimePeriodPropertyType timePeriodPropertyType = TimePeriodPropertyType.Factory
				.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
	 ...
	 *             * if an error occurs.
	 */
	private TimePeriodType createTimePeriodType(final TimePeriod timePeriod,
			TimePeriodType timePeriodType) throws OwsExceptionReport {
		if (timePeriodType == null) {
			timePeriodType = TimePeriodType.Factory
					.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
		}
		if (timePeriod.getGmlId() != null && !timePeriod.getGmlId().isEmpty()) {
			timePeriodType.setId(timePeriod.getGmlId());
		} else {
			timePeriodType.setId("tp_"
					+ JavaHelper.generateID(timePeriod.toString()
							+ System.currentTimeMillis()));
		}
		timePeriodType.setBeginPosition(createTimePositionType(timePeriod
				.getStartTimePosition()));
		timePeriodType.setEndPosition(createTimePositionType(timePeriod
				.getEndTimePosition()));

		return timePeriodType;
	}

	private XmlObject createTimeInstantDocument(final TimeInstant time)
			throws OwsExceptionReport {
		final TimeInstantDocument timeInstantDoc = TimeInstantDocument.Factory
				.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
		createTimeInstantType(time, timeInstantDoc.addNewTimeInstant());
		return timeInstantDoc;
	}

	private XmlObject createTimeInstantPropertyType(TimeInstant time)
			throws OwsExceptionReport {
		final TimeInstantPropertyType timeInstantPropertyType = TimeInstantPropertyType.Factory
				.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
	 ...
	 *             * if an error occurs.
	 */
	private TimeInstantType createTimeInstantType(
			final TimeInstant timeInstant, TimeInstantType timeInstantType)
			throws OwsExceptionReport {
		// create time instant
		if (timeInstantType == null) {
			timeInstantType = TimeInstantType.Factory
					.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
		}
		if (timeInstant.isSetGmlId()) {
			...
			timeInstantType.setId("ti_"
					+ JavaHelper.generateID(timeInstantType.toString()
							+ System.currentTimeMillis()));
		}
		timeInstantType.setTimePosition(createTimePositionType(timeInstant
				.getTimePosition()));
		return timeInstantType;
	}

	private TimePositionType createTimePositionType(
			final TimePosition timePosition) throws DateTimeFormatException {
		final TimePositionType xbTimePosition = TimePositionType.Factory
				.newInstance();
		if (!timePosition.isSetTime()) {
			if (timePosition.isSetIndeterminateValue()) {
				xbTimePosition
						.setIndeterminatePosition(TimeIndeterminateValueType.Enum
								.forString(timePosition.getIndeterminateValue()
										.name()));
			} else {
				xbTimePosition
						.setIndeterminatePosition(TimeIndeterminateValueType.Enum
								.forString(TimeIndeterminateValue.unknown
										.name()));
			}
		} else {
			final String endString = DateTimeHelper
					.formatDateTime2String(timePosition);
	...
	private XmlObject createGeomteryPropertyType(AbstractGeometry element,
			Map<HelperValues, String> additionalValues)
			throws OwsExceptionReport {
		GeometryPropertyType geometryPropertyType = GeometryPropertyType.Factory
				.newInstance();
		if (element.isReferenced()) {
			geometryPropertyType.setHref(element.getGmlId());
		} else {
			AbstractGeometryType xmlObject = createAbstractGeometry(element,
					additionalValues);
			geometryPropertyType.setAbstractGeometry(xmlObject);
			XmlHelper.substituteElement(
					geometryPropertyType.getAbstractGeometry(), xmlObject);
		}

		return geometryPropertyType;
	}

	private AbstractGeometryType createAbstractGeometry(
			AbstractGeometry element, Map<HelperValues, String> additionalValues)
			throws OwsExceptionReport {
		XmlObject xbGeometry = createPosition(element.getGeometry(),
				additionalValues);
		...
		if (xbGeometry instanceof AbstractGeometryType) {
			...
			abstractGeometryType = ((GeometryPropertyType) xbGeometry)
					.getAbstractGeometry();
		} else {
			throw new UnsupportedEncoderInputException(this, element);
		}
		...
		if (element.isSetIdentifier()) {
			abstractGeometryType
					.setIdentifier(createCodeWithAuthorityType(element
		...
		if (element.isSetDescription()) {
			abstractGeometryType.addNewDescription().setStringValue(
					element.getDescription());
		}
		return abstractGeometryType;
	}

	private XmlObject createPosition(Geometry geom,
			Map<HelperValues, String> additionalValues)
			throws OwsExceptionReport {
		String foiId = additionalValues.get(HelperValues.GMLID);
		if (geom instanceof Point) {
			final PointType xbPoint = PointType.Factory
					.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
			xbPoint.setId("point_" + foiId);
			createPointFromJtsGeometry((Point) geom, xbPoint);
			if (additionalValues.containsKey(HelperValues.DOCUMENT)) {
				PointDocument xbPointDoc = PointDocument.Factory
						.newInstance(XmlOptionsHelper.getInstance()
								.getXmlOptions());
				...
				geometryPropertyType.getAbstractGeometry().substitute(
						GmlConstants.QN_POINT_32, PointType.type);
				return geometryPropertyType;
			}
			return xbPoint;
		}
			...
			final LineStringType xbLineString = LineStringType.Factory
					.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
			xbLineString.setId("lineString_" + foiId);
			createLineStringFromJtsGeometry((LineString) geom, xbLineString);
			if (additionalValues.containsKey(HelperValues.DOCUMENT)) {
				LineStringDocument xbLineStringDoc = LineStringDocument.Factory
						.newInstance(XmlOptionsHelper.getInstance()
								.getXmlOptions());
				...
				geometryPropertyType.getAbstractGeometry().substitute(
						GmlConstants.QN_LINESTRING_32, LineStringType.type);
				return geometryPropertyType;
			}
			return xbLineString;
		}
			...
			final PolygonType xbPolygon = PolygonType.Factory
					.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
			xbPolygon.setId("polygon_" + foiId);
			createPolygonFromJtsGeometry((Polygon) geom, xbPolygon);
			if (additionalValues.containsKey(HelperValues.DOCUMENT)) {
				PolygonDocument xbPolygonDoc = PolygonDocument.Factory
						.newInstance(XmlOptionsHelper.getInstance()
								.getXmlOptions());
				...
				geometryPropertyType.getAbstractGeometry().substitute(
						GmlConstants.QN_POLYGON_32, PolygonType.type);
				return geometryPropertyType;
			}
			return xbPolygon;
		} else {
	 ...
	 *            XML Point
	 */
	private void createPointFromJtsGeometry(final Point jtsPoint,
			final PointType xbPoint) throws OwsExceptionReport {
	 ...
	 *            XML LinetSring
	 */
	private void createLineStringFromJtsGeometry(
			final LineString jtsLineString, final LineStringType xbLst)
			throws OwsExceptionReport {
	 ...
	 *            XML Polygon
	 */
	private void createPolygonFromJtsGeometry(final Polygon jtsPolygon,
			final PolygonType xbPolType) throws OwsExceptionReport {
	...
	private XmlObject createReferenceTypeForCategroyValue(
			final CategoryValue categoryValue) {
		final ReferenceType xbRef = ReferenceType.Factory
				.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
	...
	private ReferenceType createReferencType(
			final org.n52.sos.ogc.gml.ReferenceType sosReferenceType) {
		if (!sosReferenceType.isSetHref()) {
			final String exceptionText = String.format(
					"The required 'href' parameter is empty for encoding %s!",
	...
	private CodeWithAuthorityType createCodeWithAuthorityType(
			final CodeWithAuthority sosCodeWithAuthority) {
		if (!sosCodeWithAuthority.isSetValue()) {
			final String exceptionText = String.format(
					"The required 'value' parameter is empty for encoding %s!",
					CodeWithAuthorityType.class.getName());
			LOGGER.error(exceptionText);
			throw new IllegalArgumentException(exceptionText);
		}
		final CodeWithAuthorityType codeWithAuthority = CodeWithAuthorityType.Factory
				.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
	...
	private CodeType createCodeType(
			final org.n52.sos.ogc.gml.CodeType sosCodeType) {
		...
		final CodeType codeType = CodeType.Factory.newInstance(XmlOptionsHelper
	...
	protected MeasureType createMeasureType(final QuantityValue quantityValue)
			throws OwsExceptionReport {
		...
		final MeasureType measureType = MeasureType.Factory
				.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
