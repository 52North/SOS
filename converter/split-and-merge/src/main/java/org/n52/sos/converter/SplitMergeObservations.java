...
public class SplitMergeObservations implements RequestResponseModifier<AbstractServiceRequest<?>, AbstractServiceResponse> {
		...
		for (String service : services) {
			for (String version : versions) {
				for (AbstractServiceRequest<?> request : requestResponseMap
					...
					keys.add(new RequestResponseModifierKeyType(service, version, request, requestResponseMap.get(request)));
				}
			}
		}
		return keys;
	...
	private AbstractServiceResponse mergeObservations(
		...
		if (response.isSetResponseFormat()) {
			ObservationEncoder<XmlObject, OmObservation> encoder = (ObservationEncoder<XmlObject, OmObservation>)CodingHelper.getEncoder(response.getResponseFormat(), new OmObservation());
